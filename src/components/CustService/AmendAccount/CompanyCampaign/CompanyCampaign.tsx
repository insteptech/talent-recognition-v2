...
import { IcustserviceProductState, IStateType, IRootPageStateType } from "../../../../store/models/root.interface";
import {CSProductModificationStatus} from '../../../../store/models/custserviceProduct.interface'
import {
  ...
  fetchCampaigns,
  closeCampaign
} from "../../../../store/actions/CustService/custServiceProducts.action";
...
const CompanyCampaign: React.FC = () => {

  const dispatch: Dispatch<any> = useDispatch();
  const location = useLocation()
  const history = useHistory();

  const deptsAccess: IcustserviceProductState = useSelector((state: IStateType) => state.account.deptsAccess);
  const path: IRootPageStateType = useSelector((state: IStateType) => state.root.page);
  const allCampaigns: IcustserviceProductState = useSelector((state: IStateType) => state.custServiceProduct.campaigns.result);
  const totalCampaignsRecord: IcustserviceProductState = useSelector((state: any) => state.custServiceProduct.campaigns.totalRecord);

  const loader: any = useSelector((state: IStateType) => state.custServiceProduct.isLoader);
  ...
  const [selectedCampaignCS, setSelectedCampaign] = useState({ campaignID: '' });
  ...
  const handleCreate = async () => {
    ...
    await window.sessionStorage.setItem("selectedCampaignCS", '0');
    await history.push(`/customerService/createCampaign`);
  ...
  function onProductSelect(product: IcustserviceProduct): void {
    dispatch(changeSelectedProduct(product));
    dispatch(setModificationState(CSProductModificationStatus.None));
  }
  ...
  const editCampaign = (id: any) => {
    ...
    window.sessionStorage.setItem("selectedCampaignCS", id);
    history.push(`/customerService/createCampaign/${id}`);
  }
  ...
  const handleCampaign = (campaign: any) => {
    ...
    history.push('/customerService/CampaignApplications');
  }
  ...
  const confirmClose = () => {
    const campaignData = {
      id: selectedCampaignCS.campaignID,
      date: closeDate,
      comment: "",
    }
