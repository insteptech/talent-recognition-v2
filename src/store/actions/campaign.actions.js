import { store, get, storeWithouHeader, fetchWithoutHeader, ip, formPost, fetchPsy, savePsyData, postAsJson } from "../../services/api";
import { setCookie, getCookie } from "../../services/cookie";
import * as authAction from './account.actions';
import {fetchCampaigns} from './products.action';
...
export function createCampaign(data) {
    return (dispatch, getStore) => {
        ...
        const selectedCampaign = window.sessionStorage.getItem("selectedCampaign");
        const inprocessId1 = state.campaign.id || Number(selectedCampaign);
...
export function fetchCampaignMbti() {
    return (dispatch, getStore) => {
        ...
        const state = getStore();
        const selectedCampaign = window.sessionStorage.getItem("selectedCampaign");
        const inprocessId = state.campaign.id || Number(selectedCampaign);
        if (inprocessId === null && Number(inprocessId) != 0) {
            return;
        }
        get(`campaigns/mbti/${inprocessId}`)
            .then((response) => {
...
export function fetchCampaignPreferences() {
    return (dispatch, getStore) => {
        ...
        get(`campaigns/campaignpreference/${inprocessId}`)
            .then((response) => {
...
export function fetchProcessRoleSelected() {
    return (dispatch, getStore) => {
        ...
        get(`campaigns/ProcessRoleSelected/${inprocessId}`)
            .then((response) => {
...
// export function updateCampaignTypeAction(page) {
//     return (dispatch, getStore) => {
//         if (authAction.isTokenExpired()) {
//             dispatch(authAction.setExpired())
//             dispatch(authAction.setExpired(true));
//             dispatch(authAction.isUserLoggedIn(false));
//             return;
//         }
//         get('campaigns/UpdateCampaignType')
//             .then((response) => {
//                 // dispatch(updateCampaignType(page))
//             })
//             .catch((error) => {
//             })
//     }
...
export function updateCampaignTypeAction(data) {
    return (dispatch, getStore) => {
        return store(`campaigns/UpdateCampaignType`, data).then((response) => {
            return response;
        })
            .catch((error) => {
            });
...
export function fetchPreSpendRatio() {
    return (dispatch, getStore) => {
        ...
        const state = getStore();
        get('campaigns/GetPreSpendValues')
            .then((response) => {
...
export function getCampaignResult(data) {
    return (dispatch, getStore) => {
        ...
        dispatch(setLoader(SET_LOADER_CREATE_CAMPAIGN, true));
        return store(`campaign/result`, data)
            .then((response) => {
...
export function fetchCampaignDetails(id) {
    return (dispatch, getStore) => {
        ...
        const state = getStore();
        return get(`campaigns/detail/${id}`).then((response) => {
...
export function deleteDraftCampaign(id) {
    return (dispatch, getStore) => {
        ...
        return store(`campaigns/deleteDraft`, id)
        .then((response) => {
...
export function getCampaign(id) {
    return (dispatch, getStore) => {
        ...
        fetchWithoutHeader(`user/GetCompanyInfo/${id}`)
            .then((response) => {
...
export function getIp() {
    return (dispatch, getStore) => {
        return ip(`//api.ipify.org/?format=json`)
            .then((response) => {
...
export function saveApiResponse(psyResult, bfmData, count) {
    return (dispatch, getStore) => {
        ...
        savePsyData('user/SavePsyResult', escapeJson(JSON.stringify(psyResult)))
            .then((response) => {
...
export function getPycReport(data, count) {
    return dispatch => {
        ...
        fetchPsy(`psy/${data.faces[0].person.true_id}`)
            .then((response) => {
...
export function executeReport() {
    return (dispatch, getStore) => {
        ...
        postAsJson('user/ExecuteReport', Number(ApplicationID))
            .then((response) => {
