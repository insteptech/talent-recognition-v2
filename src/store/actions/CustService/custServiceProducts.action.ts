...
import { storeCS } from "../../../services/apiCS";
import { getCampaignResult } from "../../actions/CustService/custServiceCampaign.action";
...
export function addProduct(product: IcustserviceProduct): IAddProductActionType {
    return { type: ADD_PRODUCT, product: product };
}

export function editProduct(product: IcustserviceProduct): IEditProductActionType {
...
export function changeSelectedProduct(product: IcustserviceProduct): IChangeSelectedProductActionType {
...
export function setModificationState(value: CSProductModificationStatus): ISetModificationStateActionType {
...
export function fetchCampaigns(pageInfo: any) {
    ...
    return (dispatch: (arg0: any) => void, getStore: any) => {
        ...
        return storeCS(`campaigns/dashboard`, pageInfo)
            .then((response) => {
...
export function closeCampaign(data: any) {
    return (dispatch: (arg0: (dispatch: (arg0: any) => void, getStore: any) => Promise<any>) => void, getStore: () => any) => {
        ...
        return storeCS(`campaigns/close`, data)
            .then((response) => {
...
export function selectCandidate(data: any, id?: any, key?: any) {
    return (dispatch: (arg0: any) => void, getStore: any) => {
        ...
        return storeCS(`campaigns/selectCandidate`, data)
            .then((response) => {
                // dispatch(fetchCampaigns(pageInfo));
...
export function hideCandidate(data: any, id?: any, key?: any) {
    return (dispatch: (arg0: any) => void, getStore: any) => {
        ...
        dispatch(setLoader(SET_LOADER_CAMPAIGN_APPLICATION, true));
        return storeCS(`campaigns/hideCandidate`, data)
            .then((response) => {
                // dispatch(fetchCampaigns(pageInfo));
                dispatch(getCampaignResult(pageInfo));
                dispatch(setLoader(SET_LOADER_CAMPAIGN_APPLICATION, false));
...
interface IAddProductActionType { type: string, product: IcustserviceProduct };
interface IEditProductActionType { type: string, product: IcustserviceProduct };
interface IRemoveProductActionType { type: string, id: number };
interface IChangeSelectedProductActionType { type: string, product: IcustserviceProduct };
interface IClearSelectedProductActionType { type: string };
interface ISetModificationStateActionType { type: string, value: CSProductModificationStatus };
